...
import { FaWhatsapp, FaTelegramPlane, FaStickyNote, FaRegStickyNote } from "react-icons/fa";
...
const InvoiceEntries = () => {
  ...
  const fetchUserNotes = async (email) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${baseUrl}/api/user/${email}/courses/${courseId}/notes`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const addNote = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${baseUrl}/api/user/${selectedPaymentForNotes.email}/courses/${courseId}/notes`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const updateNote = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${baseUrl}/api/user/${selectedPaymentForNotes.email}/courses/${courseId}/notes/${editingNoteId}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const deleteNote = async (noteId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${baseUrl}/api/user/${selectedPaymentForNotes.email}/courses/${courseId}/notes/${noteId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`,
        },
            ...
            <tbody>
              {filteredPayments.map((payment, index) => (
                <tr key={payment._id}>
                  ...
                  <td>
                  <button
  className={`notes-btn ${hasNotes(payment.email) ? "has-notes" : "no-notes"}`}
    ...
    <div className="notes-modal">
      ...
      <div className="notes-modal-header">
        <div className="user-info">
          ...
          <div>
            <h3>{selectedPaymentForNotes.fullName || selectedPaymentForNotes.email}</h3>
      ...
      <div className="notes-input-section">
        <div className="notes-textarea-container">
          <label htmlFor="note-textarea" className="sr-only">Add a new note</label>
          <textarea
            id="note-textarea"
          ...
          <button
  className={`action-btn primary-btn ${editingNoteId ? 'update-btn' : 'add-btn'}`}
  onClick={editingNoteId ? updateNote : addNote}
  disabled={editingNoteId ? !editingNoteText.trim() : !newNote.trim()}
>
  {editingNoteId ? (
    <>
    ...
    <>
      ...
      <div className="existing-notes-section">
        <div className="section-header">
          <h4>Notes History</h4>
          <span className="notes-count">{userNotes.length} note{userNotes.length !== 1 ? 's' : ''}</span>
        </div>
        
        {userNotes.length > 0 ? (
          <div className="notes-timeline">
            {userNotes.map((note) => (
              <div key={note._id} className={`note-card ${editingNoteId === note._id ? 'editing' : ''}`}>
