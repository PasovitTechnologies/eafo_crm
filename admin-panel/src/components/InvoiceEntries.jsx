...
const InvoiceEntries = () => {
    ...
    const fetchCourseData = async () => {
      try {
        ...
        if (registrationForms.length > 0) {
          const allSubmissions = await fetchAllRegistrationSubmissions(
            registrationForms,
            token
          );
  ...
  const fetchAllUserNames = async () => {
            ...
            const fullName =
              userData.dashboardLang === "ru"
                ? `${personalDetails.lastName || ""} ${
                  ...
                  } ${personalDetails.middleName || ""}`.trim()
                : `${personalDetails.firstName || ""} ${
    ...
    console.log(
      "📲 Fetched User Names with Phone Numbers and Country:",
      namesMap
    ); // ✅ Debug log
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (course && course.payments && course.payments.length > 0) {
            const correctPayment = course.payments.find((p) => p.invoiceNumber);
            statusMap[email] = correctPayment
              ? correctPayment.status
              : "Not Created";
          } else {
            statusMap[email] = "Not Created";
          }
        } else {
          statusMap[email] = "Not Created";
        }
    ...
    await Promise.all(
      forms.map(async (form) => {
        try {
          ...
          if (!response.ok)
            throw new Error(`Failed to fetch form ${form.formId}`);

          const formData = await response.json();
          allSubmissions = [...allSubmissions, ...formData.submissions];
        } catch (error) {
          console.error(
            `❌ Error fetching submissions for form ${form.formId}:`,
            error
          );
        }
      })
          ...
          <div className="filter-icon-wrapper" ref={filterRef}>
            ...
            {showFilterModal && (
              <div className="course-filter-bubble">
                ...
                <button
                  className={`filter-option ${
                    activeFilter === "Not Created" ? "active" : ""
                  }`}
          ...
          <table className="entries-table">
            <thead>
              <tr>
                ...
                <th> {t("InvoiceEntries.paymentStatus")}</th>{" "}
                {/* ✅ New Action column */}
              </tr>
            </thead>
            ...
            <tbody>
              {filteredSubmissions.map((submission, index) => {
                ...
                let paymentStatus = paymentStatuses[email] || "Not Created";
                ...
                if (
                  paymentStatus.toLowerCase() === "invoice not created (send)"
                ) {
