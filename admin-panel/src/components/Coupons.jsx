...
const Coupons = () => {
  ...
  useEffect(() => {
    ...
    if (editorRef.current) {
      editorRef.current.innerHTML =
        currentLanguage === "en" ? messageEn : messageRu;
    }
  }, [currentLanguage]);
  ...
  const handleAddCoupon = async () => {
    ...
    if (limit !== null && (isNaN(limit) || limit < 0)) {
      setErrorMessage(
        "Total limit must be a non-negative number or left blank."
      );
      return;
    }
    ...
    if (couponType === "user" && selectedUsers.length === 0) {
      setErrorMessage(
        "Please select at least one user for user-specific coupons."
      );
      return;
    }
    ...
    const payload = {
      ...
      ...(showNotification && {
        notification: {
          message,
          type: "info",
          users: couponType === "user" ? selectedUsers.map((u) => u.uid) : [],
          isHtml: true,
        },
      }),
    };
            ...
            {couponType === "user" && (
              <div className="section">
                ...
                <div className="user-selection">
                  ...
                  {(userFilter === "old" || userFilter === "paid") &&
      ...
      <div className="coupon-list">
        ...
        {coupons.length === 0 ? (
          ...
          <ul className="coupon-items">
            {coupons.map((coupon) => {
              const userCount = coupon.users ? coupon.users.length : 0;
              const availableUsage =
                coupon.totalLimit === null
                  ? "Unlimited"
                  : Math.max(coupon.totalLimit - coupon.currentLimit, 0);
              ...
              return (
                <li key={coupon._id} className="coupon-item">
                  ...
                  <div>
                    <strong>Limit:</strong> {coupon.totalLimit ?? "Unlimited"}
                  </div>
                  <div>
                    <strong>Available Usage:</strong> {availableUsage}
                  </div>
                  {coupon.type === "user" && (
                    <div>
