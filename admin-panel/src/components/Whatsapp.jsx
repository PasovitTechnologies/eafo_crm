...
function WhatsApp() {
  ...
  const fetchChats = async (filter = "") => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/api/whatsapp/chats`,
        { params: { client_name: filter } }
      );
  
      if (response.data && response.data.dialogs) {
        console.log("✅ Fetched Chats:", response.data.dialogs);
        setChats(response.data.dialogs);
      } else {
        console.warn("⚠️ No chats found.");
        setChats([]);
      }
  ...
  const fetchMessages = async (chatId) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/api/whatsapp/chat/messages`,
        {
  ...
  const handleFileUpload = (e, type) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        console.log("Base64 File Data:", base64String.slice(0, 50) + "...");
      };
  ...
  const handleSendFile = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/api/whatsapp/document/send",
        payload,
        {
          headers: {
