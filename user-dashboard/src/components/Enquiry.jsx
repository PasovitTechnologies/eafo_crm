...
const Enquiry = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (message.length < 10) {
      toast.error(t("enquiry.messageTooShort"));
      return;
    }
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/enquiries/${selectedEnquiry}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/enquiries/${selectedEnquiry}/rating`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    ...
    >
      ...
      <div className="enquiry-page">
        {showHelpPopup && <EnquiryHelp onClose={toggleHelpPopup} />}

        <div className="breadcrumb enquiry-head-container">
          <div style={{ display: "flex", gap: "20px" }}>
            <button
              type="button"
              className="enquiry-back-icon-button"
          ...
          <div className="enquiry-detail-view">
            ...
            <div className="enquiry-detail-card">
              ...
              <div className="detail-meta">
                <span
                  className={`status ${currentEnquiry.status.toLowerCase()}`}
                >
                  {t("enquiry.status", { status: currentEnquiry.status })}
                </span>
                <span className="date">
                  {t("enquiry.created", {
                    date: new Date(currentEnquiry.createdAt).toLocaleString(),
                  })}
                </span>
                {currentEnquiry.updatedAt !== currentEnquiry.createdAt && (
                  <span className="date">
                    {t("enquiry.lastUpdated", {
                      date: new Date(currentEnquiry.updatedAt).toLocaleString(),
                    })}
                  </span>
                )}
              </div>
                ...
                <div className="detail-rating">
                  ...
                  {currentEnquiry.rating ? (
                    <div className="rating-display">
                      ...
                      <span>
                        (
                        {t("enquiry.ratingOutOf", {
                          rating: currentEnquiry.rating,
                        })}
                        )
                      </span>
                    </div>
